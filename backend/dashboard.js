...
router.put("/day/:id", (req, res) => {
  ...
  datasource
    .query(`UPDATE structure SET ${day} = ? WHERE Structure_id = ?`, [
      toggleDay,
      id,
    ])
    .then(([result]) => {
...
router.put("/places/:id", (req, res) => {
  ...
  datasource
    .query("UPDATE structure SET Nb_places = ? WHERE Structure_id = ?", [
      places,
      id,
    ])
    .then(([result]) => {
...
router.put("/hours/:id", (req, res) => {
  ...
  datasource
    .query(
      "UPDATE structure SET Heure_min = ?, Heure_max = ? WHERE Structure_id = ?",
      [heure_min, heure_max, id]
    )
    .then(([result]) => {
      if (result.affectedRows === 0) {
        res.status(404).send("Not found");
...
router.put("/indemn_repas/:id", (req, res) => {
  ...
  datasource
    .query("UPDATE structure SET Indemn_repas = ? WHERE Structure_id = ?", [
      indemn_repas,
      id,
    ])
    .then(([result]) => {
